...
func (s *AuthHandler) RefreshToken(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	request := authContract.RefreshTokenRequest{}
	...
	resp, err := s.authUseCase.RefreshToken(ctx, request)
	if err != nil {
